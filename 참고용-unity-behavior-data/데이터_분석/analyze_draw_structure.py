...
DRAW_FILE_RE = re.compile(r"(DrawRecord|DrawPreviewRecord)", re.I)
PACKAGE_RE = re.compile(r"(ItemPackage|ItemPack)", re.I)
ITEM_RE = re.compile(r"ItemRecord", re.I)
...
def flatten(o, prefix="$"):
    if isinstance(o, dict):
        for k,v in o.items():
            p=f"{prefix}.{k}"
            if isinstance(v,(str,int,float,bool)): yield p,v
...
def file_types(name):
    ...
    if "itempackage" in n or "itempack" in n: out.append("ItemPackage")
    if "itemrecord" in n: out.append("Item")
    if "rewardrecord" in n or "rewardgroup" in n: out.append("Reward")
    if any(x in n for x in ("currency","currencyrecord","wallet")): out.append("Currency")
    if any(x in n for x in ("event","season","limited")): out.append("Event")
    return out
...
def classify_field(field):
    ...
    tags=[]
    if PROB_RE.search(key): tags.append("probability_candidate")
    if COST_RE.search(key): tags.append("cost_candidate")
    if PERIOD_RE.search(key): tags.append("period_candidate")
    if PITY_RE.search(key): tags.append("pity_candidate")
    if COUNT_RE.search(key): tags.append("quantity_candidate")
    return tags
...
def main():
    ...
    index=defaultdict(list); parsed=0; errors=[]
    for p in files:
        ...
        for path,obj in walk(data):
            x=rid(obj)
            if x is not None:
                index[x].append({"id":x,"file":rel,"path":path,"types":file_types(rel),"object":obj})

    def targets(obj):
        result=[]
        for field,v in flatten(obj):
            key=field.rsplit(".",1)[-1]
            if not re.search(r"(id|ids|item|package|reward|draw|pool|currency|cost|ticket|price)",key,re.I): continue
            s=scalar(v)
            ...
            for part in re.split(r"[|,;]",s):
                ...
                if part and part in index:
                    for t in index[part]:
                        result.append({"field":field,"target_id":part,"target_file":t["file"],"target_path":t["path"],"target_types":t["types"]})
        # preserve unique edges
        seen=set(); out2=[]
        for r in result:
            k=tuple(r.items())
            if k not in seen: seen.add(k); out2.append(r)
        return out2

    draws=[]; structure=[]; prob=[]; costs=[]; periods=[]; unresolved=[]
    counts=Counter(); target_types=Counter(); field_counts=Counter()
    ...
    for e in draw_entries:
        obj=e["object"]; edges=targets(obj)
        direct_types=Counter()
        for edge in edges:
            for t in edge["target_types"]: direct_types[t]+=1; target_types[t]+=1
        row={"id":e["id"],"record_type":e["types"],"source_file":e["file"],"path":e["path"],
             "name_ko":kr_name(obj,e["file"]),"fields":[k for k in obj.keys()],
             "edges":edges,"direct_target_types":dict(direct_types)}
        draws.append(row)
        counts.update(e["types"])
        for field,value in flatten(obj):
            tags=classify_field(field)
            if not tags: continue
            item={"id":e["id"],"record_type":e["types"],"source_file":e["file"],"path":e["path"],
                  "field":field,"raw_value":value,"tags":tags}
            field_counts[field.rsplit(".",1)[-1]]+=1
            if "probability_candidate" in tags: prob.append(item)
            if "cost_candidate" in tags: costs.append(item)
            if "period_candidate" in tags: periods.append(item)

        # Mark draw records without any resolved target as unresolved.
        if not edges:
            unresolved.append({"id":e["id"],"record_type":e["types"],"source_file":e["file"],"path":e["path"],
    ...
    (out/"field_semantics_inventory.json").write_text(json.dumps({
        "probability_candidates":len(prob),"cost_candidates":len(costs),"period_candidates":len(periods),
        "top_fields":field_counts.most_common(100),
        "note":"field name alone does not confirm semantic meaning"
    },ensure_ascii=False,indent=2),encoding="utf-8")

    summary={"json_file_count":len(files),"parsed_file_count":parsed,
             "unique_record_ids":len(index),"draw_record_candidates":counts["DrawRecord"],
             "draw_preview_candidates":counts["DrawPreviewRecord"],"draw_total_entries":len(draws),
             "resolved_draw_entries":len(draws)-len(unresolved),"unresolved_draw_entries":len(unresolved),
             "target_type_counts":dict(target_types),"probability_field_occurrences":len(prob),
             "cost_field_occurrences":len(costs),"period_field_occurrences":len(periods),
             "parse_errors":len(errors),"rules":{"obscured_id":"hiddenValue XOR currentCryptoKey",
             "kr_name_field":"m_cn","semantic_inference":False}}
    (out/"00_draw_analysis_summary.json").write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding="utf-8")
    (out/"README.md").write_text("""# Gacha / Draw 4차 분석\n\nDrawRecord와 DrawPreviewRecord의 실제 Reference를 조사하는 단계다.\n\n확률/수량/가격은 필드명만으로 확정하지 않는다. 각 NDJSON에는 원본 파일/Record path/field와 대상 Reference evidence를 보존한다.\n\n다음 단계는 반복 구조를 이용해 실제 Draw → Preview → ItemPackage → Item 체인을 정제하고, 확률/비용/기간/천장 필드의 의미를 교차 검증하는 것이다.\n""",encoding="utf-8")
    md=["# Gacha 4차 구조 분석","","## 실행 요약",
    ...
    md += ["","## 해석 주의","- 현재 결과는 구조 후보다.","- 숫자 하나만으로 확률/수량/가격을 확정하지 않는다.","- 다음 단계에서 Draw/Preview/ItemPackage/Item의 반복 Reference 체인을 교차 검증한다."]
